...
public class search {
    public static void main(String[] args) {
        ...
        try {
            br = new BufferedReader(new FileReader(kb.nextLine()));// reads information from the file specified by user
                                                                   // input
            ...
            while (br.ready()) {// should repeat until there are no more lines to read
                ...
                for (int i = 0; i < result.length; i++) {
                    if (!ht.containsKey(result[i])) {
                        ...
                        ArrayList<String> temp = (ArrayList<String>) ht.get(result[i]);// if the key has already been
                                                                                       // assigned, thats ok
